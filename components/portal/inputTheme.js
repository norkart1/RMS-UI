...
function Input({ isDisabled = false, style, type = 'text', value, dropdownOpts, label, name, helper_text, handleOnChange, placeholder, status = 'normal', textAreaRowCount = "10", textAreaColCount = "30" }) {
    if (type == "text")
        return (
    ...
    if (type == "text_area")
        return (
            <div className={styles.page}>
                {label && <label htmlFor={name} >{label}</label>} <br />
                <textarea data-theme='text'  name={name}  id={name} cols={textAreaColCount} rows={textAreaRowCount} value={value} disabled={isDisabled}
                    placeholder={placeholder} data-status={status} required onChange={handleOnChange} style={style}  autoComplete='off' >
                </textarea>
                {helper_text && <p data-theme="helper" >{helper_text}</p>}
            </ div>)
    if (type == "password")
