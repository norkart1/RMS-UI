...
import { apiDelete, apiGet, apiPatch, apiPost, passwordify } from '../../../helpers/functions'
...
function Coordinators() {
    ...
    const loadTableData = async () => {
        apiGet('/admin/coordinators',)
        baseApi.get(`/admin/coordinators`, {
            headers: {
    ...
    const handleDelete = (id,) => {
        setSubmitting(true)
        apiDelete('admin/coordinators/',id, false,false, ()=>{loadTableData(); setSubmitting(false)})
                ...
                <div className={styles.dataContainer}>
                    <div className={styles.forms}>
                        ...
                        <div className={styles.formContainer} data-theme='formContainer'>
                            <form action="#" id='form'>
                                <Text_input label='First name' name='first'  handleOnChange={e => setfirstName(e.target.value)} placeholder='first name' status='normal' />
                                <Text_input label='Last name' name='lastName' handleOnChange={e => setlastName(e.target.value)} placeholder='lastName' status='normal' />
                                <Text_input label='User name' name='userName'  handleOnChange={e => setuserName(e.target.value)} placeholder='User name' status='normal' />
                                <Text_input type='password' label='Password' name='password'  status={isEmailValid == 1 ? 'success' : 'failed'} handleOnChange={e => setpassword(e.target.value)} placeholder='Password' />
                                <Text_input label='Email' name='email'  handleOnChange={e => { setEmail(e.target.value); ValidateEmail() }} placeholder='Email' status='normal' />
                                <Text_input label='Phone' name='phone' handleOnChange={e => setphone(e.target.value)} placeholder='Mobile Number' status='normal' />
                                    ...
                                    sampleData.map((item, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{item.lastName}</td>
                                                <td>{item.userName}</td>
                                                <td>{passwordify(item.password)}</td>
                                                <td>{item.email}</td>
                                                <td>{item.phone}</td>
                                                <td>{item.id}</td>
                                                <td>
