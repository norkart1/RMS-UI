...
function Categories() {
 ...
 // filteredPrograms = substractArrays(regPrograms, programs)
 let candidates;
 ...
 const handleSubmit = async (e) => {
  ...
  candDetail.map((item) => {
   setTimeout(() => {
    ...
    apiPost('coordinator/candidate-programs', data, false, false, false, () => { setSubmitting(false) })
   })
   }, 1000);
 }
     ...
     <div className={styles.tables}>
      ...
      <div data-theme="table" style={{ height: '70vh' }}>
       ...
       <Data_table id='institutesTable' heads={heads} >
        {programs &&
         programs.filter(program => program.categoryID == catID).map((program, index) => {
          let siNo = index + 1;
          return (
